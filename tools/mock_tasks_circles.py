...
def tile(t, r, c):
    ...
    if "family" not in t or t["family_state"] != st:
        ...
        t["family"] = mv.cfamily(frng, st, n=10)
        t["family_state"] = st
    frame, mark, extra = t["family"][(r * 31 + c * 17 + int(t["seed"] * 97)) % len(t["family"])]
    hue = t["hue"]
    if st == "done":
        g = mv.cglyph(frame, mark, extra, hue)
    elif st == "started":
        g = mv.cglyph(frame, mark, extra, hue if (r + c) % 2 else mv.IVORY)
    elif st == "todo":
        g = mv.cglyph(frame, mark, extra, mv.dim(hue, 0.6))
    else:
        g = mv.cglyph(frame, mark, extra, mv.dim(hue, 0.42))
...
def ring_tile(r, c):
    ...
    v = cell_rng.random()
    if v < 0.1:
        q = 0.22 * S; dr.ellipse([S / 2 - q, S / 2 - q, S / 2 + q, S / 2 + q], outline=col, width=int(1.3 * ss))
    else:
        n = cell_rng.choice([1, 2, 3])
        rad = cell_rng.choice([0.07, 0.1, 0.13]) * S
        if kind == "hline": pts = [(0.5 + (i - (n - 1) / 2) * 0.3, 0.5) for i in range(n)]
        elif kind == "vline": pts = [(0.5, 0.5 + (i - (n - 1) / 2) * 0.3) for i in range(n)]
        elif kind == "diag_up": pts = [(0.5 + (i - (n - 1) / 2) * 0.3, 0.5 - (i - (n - 1) / 2) * 0.3) for i in range(n)]
        else: pts = [(0.5 + (i - (n - 1) / 2) * 0.3, 0.5 + (i - (n - 1) / 2) * 0.3) for i in range(n)]
        for (x, y) in pts:
            dr.ellipse([x * S - rad, y * S - rad, x * S + rad, y * S + rad], fill=col)
