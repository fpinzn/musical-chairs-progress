...
for t in core:
    t["mark"] = rng.choice(MARKS)
    t["hue"] = MS_HUE[t["milestone"]] if rng.random() < 0.55 else mv.IVORY
...
def tile(t, r, c):
    ...
    if st == "done":
        ...
        mv._mark(dr, S, ss, "sqs" if ch == "sq" else ch, mv.dim(hue, 0.55), size=0.9)
    else:
        if (r * 7 + c * 13) % 4 == 0:
            return mv.finish(im, size)
        mv._mark(dr, S, ss, "Z", mv.dim(mv.IVORY, 0.38), size=0.8)
...
for t in core:
    t["bar"] = rng.random() < 0.45
    t["sparse"] = rng.random() < 0.6
...
def ring_tile(r, c):
    ...
    if rid not in ring_style:
        ring_style[rid] = (rng.random() < 0.3, rng.choice(list(MS_HUE.values())[1:]), rng.choice([(3, 2), (2, 2), (4, 1), (1, 1)]))
    coloured, hue, (on, off) = ring_style[rid]
    phase = (r + c + rid * 5) % (on + off)
    if phase >= on:
        return None
    ...
    im, dr, ss = mv.canvas(size)
    S = size * ss
    mv._mark(dr, S, ss, ch, mv.dim(hue if coloured else mv.IVORY, 0.6), size=0.9)
    return mv.finish(im, size)
